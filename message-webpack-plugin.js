...
const clear = () => (cClear(), true);

class WebpackMessages {
    constructor(opts) {
        opts = opts || {};
        this.name = opts.name || "project";
    ...
    apply(compiler) {
        ...
        const onStart = () => this.logger(`${colors.blueBright(`Building${name}...`)}`);
        const onReady = () => this.logger(`${colors.blueBright(`Ready to output ${name}... `)}`);

        compiler.plugin("compile", onStart);
        compiler.plugin("invalid", () => clear() && onStart());
